...
void init_irq_entry(int irq_num, uint32_t addr)
{
    ...
    entry->sel_segment = BASE_TSS;                     // On utilise le sélecteur défini (à adapter si besoin)
    entry->zero = 0;
    entry->type_attr = 0x8E;                           // Présent, DPL=0, type « interrupt gate » 32 bits
